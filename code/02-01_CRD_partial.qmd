...
crd_dfw <- crd_df %>%
  mutate(rep = factor(rep),
         ...
         ) %>% #integer/double to factor
  mutate(trtname = paste0(nrate_kgha, "+",krate_kgha)) #creating a column of trtname
...
ggplot(crd_dfw, aes(x = nrate_kgha,
  ...
  geom_boxplot() +
  geom_jitter() +
  facet_grid(.~krate_kgha) #same plot for different sets of your data
#nitrogen response is changing depending on k rate, negative effect when nrate/ krate is increased
```
...
Anova(crd_mod,type = 3) #explain the most complex factor first, that is, the interaction on this part
...
- Residual independence (no pattern) #they are independent because of randomization 
- Residual variance homogeneity (homoscedasticity) #variance around residuals are similar
- Residual normality (discuss this!)  #residual normality is not the same as data normality; your data may not be normal but residual could be normal
- Outlier detection (< -3 or > 3)  # within +/- 3 standard deviation, we have 99% of the data
...
ggplot(crd_resid, aes(x = .fitted,
  ...
  geom_smooth() # blue line on top of x axis (0) = no clear patter = blue line should be near zero and doesn't have a pattern such as quadratic, THUS, RESIDUALS ARE INDEPENDENT coz they have no pattern
...
ggplot(crd_resid, aes(x=.fitted, y=.studresid))+
  ...
  theme_bw()
```
Looks great! Next.  # homoscedastic - most important assumption that would determine if model fits assumptions
...
ggplot(crd_resid, aes(sample = .studresid)) +
  stat_qq() + 
  stat_qq_line() #if residuals are normal, then they shold fall on the line
```
...
ggplot(crd_resid, aes (x = .studresid)) +
  geom_density() + #density plot = outline of the top of histogram and it is less subjective
  scale_x_continuous(breaks = c(-3,0,3), limits = c(-3,3)) # peak is close to zero, heavier tails but residuals could still be considered normal
...
# h) Model means  
The next step in the workflow is extracting the model means.  #model-derived means will be diff with arithmetic means, esp. if you have a missing data, thus, we want to use model means
...
crd_means_all <- emmeans(crd_mod,
                         ~nrate_kgha:krate_kgha
                           ) #emmean = em = expected marginal means

crd_means_all
```

```{r interaction means n inside k}
crd_means_nk <- emmeans(crd_mod, ~nrate_kgha|krate_kgha) #nrate inside krate (using vertical bar "|")
...
crd_means_kn <- emmeans(crd_mod, ~krate_kgha | nrate_kgha) #krate inside nrate
...
crd_cld_all <- cld(crd_means_all,
                   reversed = T,
                   Letters = letters,
                   adjust = "none") #cld = compact latter display; i.e., doing pairwise comparison of all means # look at .group = 1 = lowest, highest = 2 ; reversed = to rearrage highest to lowers, Letters = letters, we want it letters than using numbers and in lowercase; means with the same letter are not different
...
crd_cld_nk <- cld(crd_means_nk, 
                   reversed=T, 
                   adjust="none",
               Letters=letters) #comparing nrates only within a given krate
...
crd_cld_kn <- cld(crd_means_kn, 
                   reversed=T, 
                   adjust="none",
               Letters=letters) #krates inside nrates
...
crd_cld_selected <- crd_cld_all %>%
  as.data.frame() %>%
  mutate(letter = trimws(.group)) %>% #trimws - trim wide space
...
geom_boxplot(data = crd_dfw,
  ...
  # Adding letters
geom_label(data = crd_cld_selected, aes(x=trtname,
                                        y = emmean, 
                                        label = letter
                                        ))

  #facet_grid(.~nrate_kgha) to compartmentalize or group within nrate/krate
```
